...
export default async function TutosPage() {
  await registrarVisita("/tutos");

  const productos = await listarProductos({ soloActivos: true, categoria: "TUTOS" });

  return (
    <section className="py-12">
      <h1 className="font-serif text-3xl text-olive-deep md:text-4xl">Tutos</h1>
      {productos.length === 0 ? (
